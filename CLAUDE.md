...
A prediction is uniquely keyed by `(match_id, player_id, phase)` where phase is `initial` or `late`. The Dashboard upserts on that conflict target. The `initial` pick is only writable within **`PREDICTION_WINDOW_HOURS` (10h) before kickoff** — earlier than that the match is locked. `late` is only writable while `halftime_open` and **supersedes** the initial pick (`late ?? initial` is the one that counts). `score_match` zeroes the superseded `initial` row so leaderboard sums never double-count. `matchStatus()` in `scoring.ts` derives the UI lifecycle: `scheduled → upcoming → locked → halftime → final` (`scheduled` = before the 10h window opens). **The 10h window is enforced in two places that must stay in sync**: `matchStatus()` client-side, and the `predictions_insert` / `predictions_update` RLS `WITH CHECK` policies server-side.
